...
public class AccountApplication : IAccountApplication
{
    ...
    public async Task<OperationResult> EditAsync(EditAccount command)
    {
        ...
        account?.Edit(command.Fullname, command.State, command.City, command.Mobile, command.Email?.ToLower(),
            picturePath, command.Birthday.ToGeorgianDateTime(), command.RoleId, command.FatherName, command.Gender,
            command.EducationalCenter, command.ScientificField, command.UniversityDegree, command.SeminaryDegree,
            command.Address, command.PostalCode);
        await _accountRepository.SaveChangesAsync();
        return operation.Succeeded();
    }
